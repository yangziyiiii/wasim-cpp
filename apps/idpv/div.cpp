...
int main() {
    ...
    auto initial_input = executor.convert({{"dividend_i","a"},{"divisor_i","b"},{"clk_i",1},{"clk_en_i",0},{"rst_i",0},{"divide_i",1}});
    ...
    auto c_a = smtlib_reader.lookup_symbol("main::1::dividend!0@1#2");
    auto c_b = smtlib_reader.lookup_symbol("main::1::divisor!0@1#2");
    ...
    std::cout<< v_a ->get_sort()<<std::endl;
    std::cout<< c_a ->get_sort()<<std::endl;
    

    auto v_a_extend = solver->make_term(smt::Op(smt::PrimOp::Sign_Extend,16),v_a);
    std::cout<< v_a_extend ->get_sort() << std::endl;

    auto check_a = solver->make_term(smt::Equal, v_a_extend, c_a);
    ...
    std::cout<< v_b ->get_sort()<<std::endl;
    std::cout<< c_b ->get_sort()<<std::endl;
    

    auto v_b_extend = solver->make_term(smt::Op(smt::PrimOp::Sign_Extend,24),v_b);
    std::cout<< v_b_extend ->get_sort() << std::endl;

    auto check_b = solver->make_term(smt::Equal, v_b_extend, c_b);
    ...
    auto v_ret_32 = solver-> make_term(smt::Op(smt::PrimOp::Sign_Extend,16),v_ret);
    std::cout<< v_ret_32 ->get_sort()<<std::endl;
    std::cout<< c_ret ->get_sort()<<std::endl;

    auto check_ret = solver->make_term(smt::Equal, v_ret_32, c_ret);
    ...
    if(res_ret.is_unsat()){
        ...
        std::cout<<"vexpr: "<<solver->get_value(v_ret_32)<<std::endl;
